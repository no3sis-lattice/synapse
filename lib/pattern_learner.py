...
def generate_pattern_id(pattern_type: str, content: Any) -> str:
    """
    Generate collision-resistant pattern ID using SHA256.

    Uses 128-bit truncated SHA256 hash, providing collision resistance
    up to ~10^18 patterns (compared to 16-bit = 64K, 24-bit = 16M).

    Args:
        pattern_type: Pattern type prefix (e.g., 'seq', 'err')
        content: Content to hash (will be converted to string)

    Returns:
        Pattern ID in format: {pattern_type}_{hash[:32]}
    """
    content_str = str(content)
    hash_obj = hashlib.sha256(content_str.encode('utf-8'))
    hash_hex = hash_obj.hexdigest()[:32]  # 128-bit (32 hex chars)
    return f"{pattern_type}_{hash_hex}"
...
class PatternMap:
    ...
    total_collisions: int = 0  # NEW: Track collision events
...
class PatternLearner:
    ...
    def _detect_sequence_pattern(
        ...
        # New sequence pattern - use collision-resistant hash
    ...
    def _detect_error_pattern(self, results: List[Dict[str, Any]]) -> Optional[Pattern]:
        ...
        # Use collision-resistant hash
        pattern_id = generate_pattern_id("err", error_type)
